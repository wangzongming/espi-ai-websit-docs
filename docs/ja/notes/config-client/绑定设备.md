---
title: デバイスバインディング [必須]
createTime: 2024/11/7 21:06:45
permalink: /ja/config-client/bind/
---
 
デバイスをオープンプラットフォームに涚する前に、まずはデバイスをバインドする必要があります。それ以外の場合、プラットフォームはアクティブに接続し、「します」を切断します。。

## コードを参照

基本的な基本テクニックは以下の通りです。実记のプロジェクトでは、一 のワークフローに记づいて変褉を更新します。ちなみに、フォーミュラは、プロビジョニングしてデバイスをバインドします。
