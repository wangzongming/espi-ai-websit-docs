...
permalink: /ja/guide/a4n3aaie/
---

::: ステップ
1. プロジェクトディレクトリにファイル `index.js` を作成します

2. `index.js` に時のコードを追加します (コード内のキーは自分で変更新します)
...
```
グループみ込みアクセスプラットフォームの詳細については、[内置的各平台使用教程](/example/builtin-server/)をご参照ください。

1.プロジェクトを実行する

プロジェクトディレクトリでコマンドラインツールを开き、時のコマンドを実行します。
```
node ./index.js
```
...
完成工事プロジェクトについては、[服务端配置/实例](/config-server/config/)をご参照ください。
